...
export const AdminNews = () => {
  ...
  const loadArticles = async () => {
    ...
    try {
      let query = supabase
        .from('news_articles')
        .select('id, title, slug, excerpt, published, published_at, created_at, deleted_at')
  ...
  const archiveArticle = async (id: string) => {
    if (!confirm('Archive this article? It will be hidden from the public but can be restored.')) return;
    try {
      ...
      {articles.length === 0 ? (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
          <h3 className="text-lg font-medium text-gray-900 mb-2">
            {showArchived ? 'No archived articles' : 'No articles yet'}
          </h3>
              ...
              <tbody className="divide-y divide-gray-200">
                {articles.map((article) => (
                  <tr key={article.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-900">{formatDate(article.published_at)}</td>
