...
import { Mail, MessageSquare, User, Calendar, Trash2, Eye, X, Reply } from 'lucide-react';
    ...
    <div>
      <div className="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-2">View and manage messages from the contact form</p>
        </div>
      ...
      {submissions.length > 0 && (
        <div className="mb-4">
          <input
            ...
            className="w-full sm:w-80 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
          />
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-200 flex justify-between items-start sticky top-0 bg-white">
              <div>
                <h2 className="text-xl font-bold text-gray-900">{viewingSubmission.subject}</h2>
            ...
            <div className="p-6 space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="font-medium text-gray-900">{viewingSubmission.name}</div>
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                <p className="text-gray-800 leading-relaxed whitespace-pre-wrap">{viewingSubmission.message}</p>
              </div>
              <div className="flex gap-3 pt-2">
                <a
                  href={`mailto:${viewingSubmission.email}?subject=Re: ${encodeURIComponent(viewingSubmission.subject)}`}
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
          ...
          <p className="text-gray-600">
            {searchTerm ? 'Try adjusting your search term.' : 'Messages from the contact form will appear here'}
          </p>
        </div>
      ) : (
                ...
                <tbody className="divide-y divide-gray-200">
                  {filtered.map((submission) => (
                    <tr key={submission.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4">
                        <div className="text-sm text-gray-600 truncate max-w-xs">{submission.message}</div>
                      </td>
              ...
              <div key={submission.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-600 line-clamp-3">{submission.message}</div>
                  </div>
