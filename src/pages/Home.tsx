...
const HERO_IMAGE = 'https://hckahrhomcgnvshwkabd.supabase.co/storage/v1/object/public/media/hero/wasteinstitute-image1.jpg';
const TESTIMONIAL_IMAGE = 'https://hckahrhomcgnvshwkabd.supabase.co/storage/v1/object/public/media/testimonials/wasteinstitute-image2.jpg';
const ICON_IMAGE = 'https://hckahrhomcgnvshwkabd.supabase.co/storage/v1/object/public/media/icons/white-icon.png';
...
export const HomePage = () => {
  ...
  const loadData = async () => {
    const [coursesRes, sectorsRes, newsRes, testimonialsRes] = await Promise.all([
      supabase
        .from('courses')
        .select('id, title, slug, description, featured_image, level, duration, price')
        .eq('published', true)
        .is('deleted_at', null)
        .limit(3),
      supabase
      ...
      supabase
        .from('testimonials')
        .select('id, student_name, student_title, student_image, rating, testimonial_text')
        .eq('published', true)
        .eq('featured', true)
        .order('display_order')
        .limit(6),
  ...
  const avgRating = useMemo(() => {
    ...
    return (testimonials.reduce((sum, t) => sum + (t.rating || 5), 0) / testimonials.length).toFixed(1);
  }, [testimonials]);
            ...
            <div className="relative">
              <div className="relative rounded-2xl overflow-hidden shadow-2xl">
                ...
                <div className="absolute inset-0 pointer-events-none">
                  <Sparkles size={30} className="absolute top-12 left-8 text-yellow-300 fill-current animate-float-slow" />
                  <Sparkles size={24} className="absolute top-24 right-16 text-yellow-400 fill-current animate-float-delayed" />
                  <Sparkles size={27} className="absolute bottom-32 left-12 text-yellow-300 fill-current animate-float" />
                </div>

                <div className="absolute bottom-6 left-6 bg-black/70 backdrop-blur-sm rounded-xl p-5 shadow-2xl max-w-xs border border-white/20">
                  <div className="flex items-center gap-2 mb-2">
                    <div className="w-2 h-2 bg-green-400 rounded-full animate-pulse"></div>
                    <span className="text-xs text-gray-300">Active Learners</span>
                  </div>
                  <p className="text-2xl font-handwriting text-white font-semibold">
                    "So excited to start!"
                  </p>
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
              {sectors.map((sector) => (
                ...
                >
                  <div className="text-4xl mb-3">{sector.icon || '\u{1F4DA}'}</div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {courses.map((course) => (
                <Link
                  ...
                  className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-xl transition-shadow border border-gray-200 hover:border-emerald-500"
                >
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-emerald-600 font-semibold">Learn More &rarr;</span>
                    </div>
                  </div>
                </Link>
              ))}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
              <div className="relative order-2 lg:order-1">
                <div className="relative rounded-2xl overflow-hidden shadow-2xl">
                  ...
                  <div className="absolute inset-0 pointer-events-none">
                    <Sparkles size={27} className="absolute top-16 right-12 text-yellow-300 fill-current animate-float" />
                    <Sparkles size={24} className="absolute top-32 left-16 text-yellow-400 fill-current animate-float-delayed" />
                  </div>

                  <div className="absolute top-6 right-6 bg-black/70 backdrop-blur-sm rounded-xl p-5 shadow-2xl border border-white/20">
                    <div className="flex items-center gap-2 mb-2">
                      <div className="w-2 h-2 bg-green-400 rounded-full animate-pulse"></div>
                      <span className="text-xs text-gray-300">Community Active</span>
                    </div>
                    <p className="text-xl font-handwriting text-white font-semibold">
                      "Amazing community!"
                    </p>
                    <div className="flex gap-3 mt-3">
                      <Users size={32} className="text-emerald-400" />
                  ...
                  <div className="animate-scroll-slow space-y-6">
                    {testimonials.concat(testimonials).map((testimonial, idx) => (
                      <div
                        ...
                        className="bg-white rounded-xl shadow-md p-6 hover:shadow-xl transition-shadow border border-gray-200"
                      >
                ...
                <div key={categoryIndex}>
                  ...
                  <div className="space-y-3">
                    {category.questions.map((faq, faqIndex) => {
                      const globalIndex = FAQ_DATA.slice(0, categoryIndex).reduce((acc, cat) => acc + cat.questions.length, 0) + faqIndex;
                      const isOpen = openFaqIndex === globalIndex;

                      return (
                        <div
                          key={faqIndex}
                          className="bg-white border border-gray-200 rounded-xl overflow-hidden hover:border-emerald-500 transition-colors shadow-sm hover:shadow-md"
                        >
                          <button
